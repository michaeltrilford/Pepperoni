...
export const BrandDoc: React.FC = () => {
  const [activePlatform, setActivePlatform] = useState<"css" | "ios" | "android" | "figma-css" | "figma-ios">("css");
    ...
    <div {...sx.props(styles.wrapper)} data-tokens-doc="">
      <div {...sx.props(styles.root)}>
        ...
        <Box flex direction="column" gap="600">
          <header {...sx.props(styles.sectionHeader)}>
            <Heading size="h1" level="h1">
              Brand
            </Heading>
            <Text size="l" variant="secondary">
              Auto-generated multi-platform brand tokens defined in W3C format (OKLCH colours, spacing, radius,
              typography, stroke, speed).
            </Text>
          </header>
          ...
          <Card size="m" surface="100" surfaceDirection="lift">
            <Box flex direction="column" gap="400">
              <Box flex direction="row" gap="100" align="start" justify="between">
                <Box flex direction="column" gap="100" style={{ maxWidth: "55ch" }}>
                  <Heading size="h4" level="h2" weight={600}>
                    Multi-Platform Token Artifacts
                  </Heading>
                  <Text size="m" variant="secondary">
                    Style Dictionary compiles Web, iOS, and Android artifacts plus dedicated DTCG imports for the Figma
                    web and mobile workspaces.
                  </Text>
                </Box>
                ...
                <Box flex direction="row" gap="100">
                  <select
                    ...
                    onChange={(event) => setActivePlatform(event.target.value as typeof activePlatform)}
            ...
            <Box flex direction="column" gap="500">
              ...
              <Box flex direction="column" gap="400">
                ...
                <div {...sx.props(styles.baseGrid)}>
                  <Card surface="100" surfaceDirection="depth" padding={false} style={{ overflow: "hidden" }}>
                    <div {...sx.props(styles.swatch)} style={{ backgroundColor: baseTokens.black.$value }} />
                    <div {...sx.props(styles.tokenMeta)}>
                      <Text size="s" weight={600} {...sx.props(styles.tokenName)}>
                        --black
                      </Text>
                      <Text size="s" variant="secondary" {...sx.props(styles.smallMeta)}>
                        {baseTokens.black.$value}
                      </Text>
                    </div>
                  </Card>
                  <Card surface="100" surfaceDirection="depth" padding={false} style={{ overflow: "hidden" }}>
                    <div {...sx.props(styles.whiteSwatch)} style={{ backgroundColor: baseTokens.white.$value }} />
                    <div {...sx.props(styles.tokenMeta)}>
                      <Text size="s" weight={600} {...sx.props(styles.tokenName)}>
                        --white
                      </Text>
                ...
                return (
                  <Box flex direction="column" gap="400" key={group.key}>
                    ...
                    <div {...sx.props(styles.colorGrid)}>
                      {Object.entries(groupObj).map(([step, item]: [string, any]) => (
                        <Card
                          key={step}
                          surface="100"
                          surfaceDirection="depth"
                          padding={false}
                          style={{ overflow: "hidden" }}
                        >
                          <div {...sx.props(styles.colorSwatch)} style={{ backgroundColor: item.$value }} />
            ...
            <Box flex direction="column" gap="500">
              ...
              <div {...sx.props(styles.typographyDetailsStack)}>
                <Box flex direction="column" gap="300">
                  ...
                  <Card surface="100" surfaceDirection="lift" padding={false}>
                    <Table usage="card" size="m" aria-label="Brand font weights">
                      ...
                      <tbody>
                        {Object.entries(fontWeights).map(([key, item]: [string, any]) => (
                          <tr key={key}>
                            <Table.Cell>
                              <Text size="s">
                            ...
                            <Table.Cell align="end" style={{ fontWeight: item.$value }}>
                              Pepperoni
                            </Table.Cell>
                          </tr>
                        ))}
                ...
                <Box flex direction="column" gap="300">
                  ...
                  <Card surface="100" surfaceDirection="lift" padding={false}>
                    <Table usage="card" size="m" aria-label="Brand letter spacing">
                      <thead>
                        <tr>
                          <Table.HeaderCell>Token</Table.HeaderCell>
                          <Table.HeaderCell align="end">Preview</Table.HeaderCell>
                      ...
                      <tbody>
                        {Object.entries(letterSpacings).map(([key, item]: [string, any]) => (
                          <tr key={key}>
                            <Table.Cell>
                              <Text size="s">
                            ...
                            <Table.Cell align="end" style={{ letterSpacing: item.$value }}>
                              Pepperoni
                            </Table.Cell>
                          </tr>
                        ))}
            ...
            <Box flex direction="column" gap="500">
              ...
              <div {...sx.props(styles.radiusGrid)}>
                {Object.entries(radii).map(([key, item]: [string, any]) => (
                  <Card key={key} surface="100" surfaceDirection="depth" size="s" style={{ textAlign: "center" }}>
                    <div {...sx.props(styles.radiusPreview)} style={{ borderRadius: item.$value }} />
                    <Text size="s" weight={600} {...sx.props(styles.radiusName)}>
                      --radius-{key}
                    </Text>
                    <Text size="s" variant="secondary" {...sx.props(styles.radiusValue)}>
                      {item.$value}
                    </Text>
                  </Card>
                ))}
            ...
            <Box flex direction="column" gap="500">
              ...
              <Card surface="100" surfaceDirection="lift" padding={false}>
                ...
                >
                  ...
                  <tbody>
                    {Object.entries(fontSizes).map(([key, item]: [string, any]) => {
                      ...
                      return (
                        <tr key={key}>
                          ...
                          <Table.Cell>
                            <Text size="s" variant="secondary">
                              {lhItem ? `--line-height-${key} (${lhItem.$value})` : "-"}
                            </Text>
                          </Table.Cell>
                          <Table.Cell
                            align="end"
                    ...
                    {Object.entries(strokes).map(([key, item]: [string, any]) => {
                      return (
                        <tr key={key}>
                          ...
                          <Table.Cell {...sx.props(styles.preview)} style={{ height: item.$value }}>
                            <div {...sx.props(styles.strokeBar)} style={{ height: item.$value }} />
                          </Table.Cell>
                        </tr>
                      );
                    })}
            ...
            <Box flex direction="column" gap="500">
              ...
              <div {...sx.props(styles.speedGrid)}>
                {Object.entries(speeds).map(([key, item]: [string, any]) => (
                  <Card key={key} surface="100" surfaceDirection="depth" size="s">
                    <Text size="s" weight={600} {...sx.props(styles.speedName)}>
                      --speed-{key}
                    </Text>
                    <Text size="s" variant="secondary" {...sx.props(styles.speedValue)}>
                      {item.$value}
                    </Text>
